...
export class AppComponent {
  ...
  constructor(private sidenavService: SidenavService,
    ...
    router.events.subscribe(event => {
      console.log(event)
      if (event instanceof NavigationStart && (event as NavigationStart).url != '/'){
      ...
      if (event instanceof NavigationEnd && (event as NavigationEnd).url != '/' && global.user == null)
      {
        baseService.getCurrentUser().subscribe(data=>{
          global.user = data;
          if (data == null){
            messageBox.showMessageBox(MessageBoxType.Warning, global.screen['Por_favor_conectese_nuevamente']);
            router.navigate(['']);
          }
        });
      }

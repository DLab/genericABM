...
export class ListMantenedorComponent implements OnInit, AfterViewInit {
  ...
  cancelarInvoker:Invoker = new Invoker(this, 'cancelarItem');
  hasFilter:boolean;
  searchResult:any[];
  ...
  private cancelarFunction:string;
  ...
  public initialize(hasFilter:boolean
      ...
      this.cancelarFunction = cancelarFunction;
      ...
      this.isValidCreateFn = isValidCreateFn;
  ...
  _addItem(_data:any){
    ...
    //console.log(this.parent.getEditDialogSize)
    if (this.parent && this.parent.getEditDialogSize){
      let size:any = this.parent.getEditDialogSize();
      params['height'] = size.height;
      params['width'] = size.width;
    }
    this.dialog.open(this.editItemDialog, params).afterClosed().subscribe(newData => {
  ...
  showEditItem(data:any, edit:boolean, copy:boolean){
    ...
    this.dialog.open(this.editItemDialog, params).afterClosed().subscribe(result => {
      if ((edit || copy) && result != undefined && result != true){
        if (edit){
  ...
  cancelarItem(data:any):void{
    this.messageBox.showMessageBox(MessageBoxType.Question, global.screen['Do_you_want_to_cancel_the_selected_process']).subscribe(value=>{
      if (value === 'SI'){
        delete data.hsArray;
        delete data.searchResult;
        data.isNew = false;
        data.codigoEstado = 'CANCEL';
        this.baseService[this.cancelarFunction](data).subscribe(resp=>{
          if (this)
          this.lastSearch();
          this.messageBox.showMessageBox(resp.status ? MessageBoxType.Success: MessageBoxType.Error, this.app.screen[resp.msg]);
        });
      }
    });
  }
  ...
  isValidCreate(){
    return this.isValidCreateFn ? this.isValidCreateFn() : true;
  }
  download(){
    var cols = [];

...
export class ActionCellComponent  implements OnInit {
  ...
  hasCommand(action:string, row:any):boolean {
    ...
    if (action === 'Copiar' || action == 'Cancel'){
      return true;
    }
    if (row.codigoEstado == undefined || row.codigoEstado == 'INGR'){
      ...
      if (row.correlativo != undefined && row.maxCorrelativo != undefined && row.correlativo == row.maxCorrelativo){
        return action === 'Visualizar' || action === 'Editar' || action === 'Desactivar' || action === 'Cancel';
      }
      return action === 'Visualizar' || action === 'Desactivar' || action == 'Cancel';
    }
  ...
  _cancel(data:any){
    if (global.currentComponent && global.currentComponent.cancelItem){
      global.currentComponent.cancelItem(data);
    }
    else if (this.cancelar){
      this.cancelar.invokeMethod(data);
    }
  }
}

...
export var global = {
  ...
  export function getTime(n:number){
    let h:number = Math.trunc(n /3600);
    n = n - h * 3600;
    let m:number = Math.trunc(n /60);
    n = Math.round(n - m * 60);
    return digitFormat(h) + ':' + digitFormat(m) + ':' + digitFormat(n);
  }
...
export const Constantes= {
  DASHBOARD_ID: 35,
    ...
    booleanArrays: [{id: '1', description:'True'}, {id: '0', description:'False'}]
};

...
public class UsuarioService extends BaseService
{
	...
	public HashMap<String, Object> saveUserConfiguration(HashMap<String, Object> input) throws Exception
	{
		...
		try
		{
			JSONObject json = new JSONObject((HashMap<String, Object>)input.get("configurations"));
			System.out.println("config:" + input.get("configurations"));
			System.out.println("json" + json.toString());
			PreparedStatement stmt = usr.getConnection().prepareStatement("update usuario set usr_configurations = ? where usr_id = ?");
			stmt.setString(1, json.toString());
			stmt.setString(2, (String)input.get("user"));
			stmt.executeUpdate();
			usr.getConnection().commit();
			return input;
		}
